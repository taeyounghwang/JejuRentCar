...
<div class="contentContainer">
	...
	<div class="table-responsive">
		...
		<table class="table table-bordered">
			<thead>
				<tr>
				<th data-value="ref_number" class="order">환불번호<c:choose>
						<c:when
							test="${data.order_by=='ref_number' and data.order_sc=='ASC' }">▲</c:when>
						<c:when
							test="${data.order_by=='ref_number' and data.order_sc=='DESC' }">▼</c:when>
						<c:otherwise>▲</c:otherwise>
					</c:choose></th>
					<th class="order">예약번호</th>
				...
				<c:choose>
					<c:when test="${not empty refundList }">
						<c:forEach var="REFUND" items="${refundList}">
							<tr data-num="${REFUND.ref_number }">
								<td><a
									href="${path}/admin/refund/refundDetail?ref_number=${REFUND.ref_number }">${REFUND.ref_number }</a>
								</td>
								<td>${REFUND.rsv_number }
								<td>${REFUND.mem_id }</td>
